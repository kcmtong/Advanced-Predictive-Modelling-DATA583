...
# model_np <- npregbw(Life.expectancy ~ Status + Adult.Mortality + infant.deaths + Hepatitis.B + BMI + under.five.deaths + Polio + Diphtheria + HIV.AIDS + GDP + thinness..1.19.years + Income.composition.of.resources + Schooling, data = df, regtype="ll", bwmethod = "cv.aic")
...
my_model <- readRDS("model_np.rds")

summary(my_model)
```

```{r cache=TRUE}
npsigtest_npreg <- npsigtest(model_np)
```

3. need to pull the LASSO code  into our final report

```{r}
library(glmnet)
y <- df$Life.expectancy
x <- data.matrix(df[, c('Year','Adult.Mortality','infant.deaths','Alcohol','Hepatitis.B','Measles','BMI','under.five.deaths','Polio','Total.expenditure','Diphtheria','HIV.AIDS','GDP','Population','thinness..1.19.years','thinness.5.9.years','Income.composition.of.resources','Schooling','Status.val')])
